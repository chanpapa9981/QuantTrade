...
class ConfigLoaderTestCase(unittest.TestCase):
    def test_load_settings_reads_yaml(self) -> None:
        ...
        self.assertFalse(settings.maintenance.enabled)
        self.assertEqual(settings.maintenance.runner_id, "maintenance-default")
        self.assertEqual(settings.maintenance.poll_interval_seconds, 300.0)
