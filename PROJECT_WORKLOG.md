...
| 项目 | 内容 |
| :--- | :--- |
| 目标 | 把历史页从“信息都在，但像原始导出报表”重做成更像正式运维软件的 UI 结构 |
| 输入 | 历史页虽然功能已经很多，但顶部指标被无差别平铺、正文分区主次弱、表格视觉像原始数据库导出，很难让人第一眼快速判断系统状态，也缺少真正产品化的质感 |
| 产出 | 历史页新的 hero 区、跳转导航、主状态卡、分组指标带、统一的 panel/table 视觉体系、改进后的留白和层级；不改变已有数据能力和交互 id，但显著改善信息架构和产品观感 |
| 结果 | 现在页面打开后先看到控制器问题、关键告警、broker drift、runner 停滞这些“主状态”，再往下看按 Runtime / Runner / Broker and Alerts / Research 分组的指标带，正文表格也从“裸表格堆叠”变成了更像正式运维台的阅读节奏 |
| 为什么这么做 | 因为真正的软件 UI 不是把所有信息展示出来就够了，而是要让人先看到最重要的状态，再决定是否继续下钻。对于运维型页面，差的排版会直接放大认知负担；只有把主次、分组、节奏做好，前面已经做出的那些 execution、notification、broker、maintenance 能力才真正可用。 |
| 未完成 | 单次 backtest dashboard 的同级 UI 重构、图表区更强的视觉表达、表格列折叠/密度切换、真正交互式前端而不是静态 HTML |
| 备注 | 这一轮主要优化的是“正式软件感”和“可读性”，不是新增数据能力 |

### 2026-03-28 第 66 轮

| 项目 | 内容 |
| :--- | :--- |
| 目标 | 把历史页继续从“好看一点的长页面”推进成“更像 QuantConnect 的工作台产品壳子” |
| 输入 | 第 65 轮已经把视觉层级改善了，但整体仍然是单页长滚动结构；它看起来比以前整齐，却还没有形成真正的产品操作框架，和 QuantConnect 那种左侧稳定导航、按工作区切换的习惯仍然差很远 |
| 产出 | 左侧 workspace 导航；`Overview / Research / Backtest / Live / Broker / Alerts / Settings` 模块壳子；顶部 active workspace bar；基于 `data-workspaces` 的前端分区显示；Research/Settings 引导面板；对应 HTML 断言测试增强 |
| 结果 | 现在历史页不只是“把很多 panel 排在一起”，而是已经有了更接近交易工作台的软件骨架：用户可以按职能切换工作区，而不是只能上下滚页面；这使当前静态页面第一次具备了明显的“产品流程感”，也为后续演进成真正的 Web app 打下了结构基础 |
| 为什么这么做 | 因为普通用户使用自动交易产品时，真正依赖的是稳定的操作路径，而不是知道某张长页面里哪一段藏着什么信息。借用 QuantConnect 的工作区框架，不是为了照抄复杂功能，而是为了直接采用成熟的用户习惯：先进某个工作区，再在该工作区里完成一类任务。只有先把壳子做对，后面继续加功能才不会越做越像报表堆。 |
| 未完成 | 真正多页面或前端路由、项目级状态持久化、单次回测 dashboard 与 history 页统一壳子、Research 真实交互面板、Settings 真正配置编辑能力 |
| 备注 | 这一轮开始从“优化页面”升级到“重塑产品框架”，方向已经切到 QuantConnect 风格的简化版交易工作台 |
